...
install() {
	...
	#
	local tmpf=$(mktemp)
	local tmpp="lk23jlkjflsa9f209jfsdlkfjlvfsaf"
	dd if=/dev/zero of="${tmpf}" bs=1048576 count=64 2>/dev/null
	zpool create -m none "${tmpp}" "${tmpf}"
	zpool destroy "${tmpp}"
	rm -f "${tmpf}"
	...
	if grep -q 'hookdir.*shutdown' "${basedir}/dracut-functions" ; then
	...
	local spl_tmp=$(mktemp)
	AA=$(hostid | cut -b 1,2)
	BB=$(hostid | cut -b 3,4)
	CC=$(hostid | cut -b 5,6)
	DD=$(hostid | cut -b 7,8)
	printf "\x$DD\x$CC\x$BB\x$AA" > "${spl_tmp}"
	inst_simple "${spl_tmp}" /etc/hostid
	rm "${spl_tmp}"
}
